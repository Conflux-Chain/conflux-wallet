...
function homeReducer(state = initialState, action) {
  switch (action.type) {
    ...
    case GENERATE_WALLET_ERROR:
      return state.set('generateWalletLoading', false).set('generateWalletError', action.error);
    ...
    case CHANGE_USER_SEED:
      return state.set('userSeed', action.userSeed); // Delete prefixed space from user seed
    case CHANGE_USER_PASSWORD:
      return state.set('userPassword', action.password);
    case RESTORE_WALLET_FROM_SEED:
      return state.set('restoreWalletError', false).set('isComfirmed', false);
    case RESTORE_WALLET_FROM_SEED_ERROR:
      return state.set('restoreWalletError', action.error);
    ...
    case CHANGE_BALANCE:
      return state.setIn(['addressList', action.address, action.symbol, 'balance'], action.balance);

    case SHOW_SEND_TOKEN:
      return state.set('isShowSendToken', true);
    case HIDE_SEND_TOKEN:
      return state.set('isShowSendToken', false);

    case SHOW_TOKEN_CHOOSER:
      return state.set('isShowTokenChooser', true);
    case HIDE_TOKEN_CHOOSER:
      return state.set('isShowTokenChooser', false);
    ...
    case GENERATE_ADDRESS_ERROR:
      return state.set('addressListLoading', false).set('addressListError', action.error);

    case LOCK_WALLET:
      return state.set('password', false);
    case UNLOCK_WALLET:
      return state;
    ...
    case SET_EXCHANGE_RATES:
      return state.set('exchangeRates', fromJS(action.rates));
    case SELECT_CURRENCY:
      return state.set('convertTo', fromJS(action.convertTo));

    case CLOSE_WALLET:
      return initialState;

    case CHECK_LOCAL_STORAGE:
      return state.set('checkLocalStorageLoading', true);
    case LOCAL_STORAGE_EXIST:
      return state.set('checkLocalStorageLoading', false).set('isLocalStorageWallet', true);
    case LOCAL_STORAGE_NOT_EXIST:
      return state.set('checkLocalStorageLoading', false).set('isLocalStorageWallet', false);

    case SAVE_WALLET:
      return state.set('saveWalletLoading', true).set('saveWalletError', false);
    case SAVE_WALLET_SUCCESS:
      return state.set('saveWalletLoading', false);
    case SAVE_WALLET_ERROR:
      return state.set('saveWalletLoading', false).set('saveWalletError', action.error);

    case LOAD_WALLET:
      return state.set('loadWalletLoading', true).set('loadWalletError', false);
